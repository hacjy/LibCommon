...
public class GameListViewPresenter implements IHttpPresenter {
    /**
     * 列表对象
     */
    ...
    public GameListViewPresenter(IRecyclerLoadView recyclerLoadView) {
    ...
    public void initRequestData() {
        if (refreshing)
            return;
        refreshing = true;
        LoadingDialog.showDialog(((GameListView)mRecyclerLoadView).getContext());
        Log.e("requestData,type","执行了,"+type);
        requestData(0);
    }

    public void onItemClick(int position){
        ToastUtil.showToast(((GameListView)mRecyclerLoadView).getContext(),"点击了position="+position);
    }

    private void requestData(final int page){
        ((GameListView) mRecyclerLoadView).postDelayed(new Runnable() {
            @Override
            public void run() {
